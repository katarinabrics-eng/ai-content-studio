...
export default function AdminDashboard() {
  ...
  return (
    <div style={{ display: "flex", flex: 1, minHeight: "100vh", minWidth: 0, flexDirection: "column" }}>
      {/* Top bar */}
      <div style={{ borderBottom: "1px solid rgba(0,0,0,0.12)", padding: "0 24px", display: "flex", alignItems: "center", gap: 16, height: 52, background: "rgba(245,244,240,0.92)", flexShrink: 0 }}>
        <div style={{ fontWeight: 700, fontSize: 15, letterSpacing: "0.05em", color: "#111" }}>LUCIFERA</div>
      ...
      <div style={{ display: "flex", flex: 1, overflow: "hidden" }}>
        ...
        <div style={{ width: 260, borderRight: "1px solid rgba(0,0,0,0.12)", background: "#f7f7f7", padding: "16px 0", flexShrink: 0, display: "flex", flexDirection: "column" }}>
          <div style={{ padding: "0 16px 12px", fontSize: 11, color: "#555", letterSpacing: "0.1em" }}>KLIENTI · {rows.length}</div>
          <button
            type="button"
            onClick={() => { setLoading(true); fetch("/api/admin/data").then((r) => r.json()).then((d) => { setRows(d.rows ?? []); setError(d.error ?? ""); setLoading(false); }); }}
            style={{ margin: "0 12px 12px", padding: "8px 12px", borderRadius: 8, border: "1px solid rgba(0,0,0,0.12)", background: "#fff", color: "#555", fontSize: 12, cursor: "pointer" }}
          >
            ↻ Obnovit
          </button>
          <input
            ...
            style={{ margin: "0 12px 12px", padding: "10px 12px", background: "#fff", border: "1px solid rgba(0,0,0,0.12)", borderRadius: 8, color: "#111", fontSize: 13, boxSizing: "border-box" }}
          />
            ...
            {!loading && filtered.map((row) => (
              <div
                ...
                style={{
                  ...
                  padding: "10px 12px",
                  borderRadius: 8,
                  background: selectedId === row.id ? "rgba(168,216,0,0.12)" : "transparent",
                  border: selectedId === row.id ? `1px solid ${COLORS.lime}` : "1px solid transparent",
                  cursor: "pointer",
                }}
              >
                <div style={{ fontSize: 13, fontWeight: 600, color: selectedId === row.id ? "#111" : "#555", marginBottom: 3 }}>{row.name || row.email || "—"}</div>
                ...
                <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
                  <span style={{ padding: "2px 7px", borderRadius: 5, fontSize: 10, fontWeight: 600, color: statusColor(row.workflow_status), background: "rgba(0,0,0,0.05)", border: `1px solid ${statusColor(row.workflow_status)}44` }}>{statusLabel(row.workflow_status)}</span>
                  <span style={{ padding: "2px 7px", borderRadius: 5, fontSize: 10, fontWeight: 600, color: "#555", background: "rgba(0,0,0,0.05)" }}>{(row.scan_result as ScanResult)?.brandScore?.total ?? "—"}/100</span>
                </div>
              </div>
            ))}
          </div>
          <a href="/diagnostika" style={{ margin: "12px 12px 0", padding: "8px 12px", borderRadius: 8, border: "1px dashed rgba(0,0,0,0.2)", fontSize: 12, color: "#555", textAlign: "center", display: "block", textDecoration: "none" }}>+ Nová diagnostika</a>
        </div>

        {/* Main content */}
        <div style={{ flex: 1, overflow: "auto", padding: "24px 28px", background: "#f8f8f8" }}>
          {!selected ? (
            <div style={{ color: "#555", padding: 48, textAlign: "center", fontSize: 15 }}>Vyberte záznam v seznamu vlevo.</div>
          ) : (
            <>
              {/* Client header */}
              <div style={{ display: "flex", alignItems: "flex-start", gap: 16, marginBottom: 24, paddingBottom: 20, borderBottom: "1px solid rgba(0,0,0,0.12)" }}>
                <div style={{ width: 44, height: 44, borderRadius: 10, background: "linear-gradient(135deg, #333, #555)", border: "1px solid rgba(0,0,0,0.1)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18, color: "#fff", flexShrink: 0 }}>
                  {(selected.name || selected.email || "?").charAt(0).toUpperCase()}
                </div>
                <div style={{ flex: 1 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6 }}>
                    <h1 style={{ margin: 0, fontSize: 20, fontWeight: 700, color: "#111" }}>{selected.name || selected.email || "—"}</h1>
                    <span style={{ padding: "3px 10px", borderRadius: 20, fontSize: 11, fontWeight: 600, color: statusColor(selected.workflow_status), background: "rgba(0,0,0,0.05)", border: `1px solid ${statusColor(selected.workflow_status)}44` }}>{statusLabel(selected.workflow_status)}</span>
                  </div>
                  <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                    {(selected.scan_result as ScanResult)?.brandDna?.contentPillars?.map((t) => (
                      <span key={t} style={{ padding: "2px 7px", borderRadius: 5, fontSize: 10, fontWeight: 600, color: COLORS.lilac, background: "rgba(124,77,190,0.1)", border: `1px solid ${COLORS.lilac}44` }}>#{t}</span>
                    )) ?? null}
                ...
                <div style={{ position: "relative", width: 60, height: 60, flexShrink: 0 }}>
                  <svg width="60" height="60" style={{ transform: "rotate(-90deg)" }}>
                    <circle cx="30" cy="30" r={22} fill="none" stroke="rgba(0,0,0,0.1)" strokeWidth="4" />
                  ...
                  <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
                    <div style={{ fontSize: 14, fontWeight: 700, color: "#111", lineHeight: 1 }}>{(selected.scan_result as ScanResult)?.brandScore?.total ?? "—"}</div>
              ...
              {/* Tabs */}
              <div style={{ display: "flex", gap: 2, marginBottom: 24, background: "rgba(0,0,0,0.06)", borderRadius: 10, padding: 4 }}>
              ...
              {activeTab === "prehled" && (
                <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
                  {(selected.scan_result as ScanResult)?.suggested_strategists?.length ? (
                    <div style={{ borderRadius: 12, border: "1px solid rgba(0,0,0,0.12)", overflow: "hidden" }}>
                      <div style={{ padding: "10px 16px", background: "#f0f0f0", borderBottom: "1px solid rgba(0,0,0,0.12)", display: "flex", alignItems: "center", gap: 8 }}>
                        <div style={{ width: 3, height: 14, borderRadius: 2, background: COLORS.purple }} />
                        <span style={{ fontSize: 12, fontWeight: 600, color: "#555", letterSpacing: "0.05em" }}>🤖 AI doporučuje pro tuto značku</span>
                      </div>
                      <div style={{ padding: 16, background: "#fff", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                        {(selected.scan_result as ScanResult).suggested_strategists!.map((s) => (
                          <div key={s.id} style={{ padding: 14, borderRadius: 10, background: "#f8f8f8", border: "1px solid rgba(0,0,0,0.1)", position: "relative" }}>
                            ...
                            <div style={{ fontSize: 14, fontWeight: 600, color: "#111", marginBottom: 4, paddingRight: 60 }}>{s.label}</div>
                  ...
                  <div style={{ borderRadius: 12, border: "1px solid rgba(0,0,0,0.12)", overflow: "hidden" }}>
                    <div style={{ padding: "10px 16px", background: "#f0f0f0", borderBottom: "1px solid rgba(0,0,0,0.12)", display: "flex", alignItems: "center", gap: 8 }}>
                      <div style={{ width: 3, height: 14, borderRadius: 2, background: COLORS.purple }} />
                      <span style={{ fontSize: 12, fontWeight: 600, color: "#555", letterSpacing: "0.05em" }}>📐 Pilíře značky</span>
                    </div>
                    <div style={{ padding: 16, background: "#fff", display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 10 }}>
                      {PILLARS.map((p) => {
                        const val = (selected.scan_result as ScanResult)?.pillarAnalysis?.[p.id];
                        const score = typeof val?.score === "number" ? val.score : 5;
                        const col = scoreColor(score);
                        return (
                          <div key={p.id} style={{ padding: "12px 10px", borderRadius: 10, background: "#f8f8f8", border: `1px solid ${col}44`, textAlign: "center" }}>
                            ...
                            <div style={{ height: 3, borderRadius: 2, background: "rgba(0,0,0,0.1)", overflow: "hidden", marginBottom: 6 }}><div style={{ height: "100%", width: `${score * 10}%`, background: col }} /></div>
                            <span style={{ padding: "2px 7px", borderRadius: 5, fontSize: 10, fontWeight: 600, color: col, background: "rgba(0,0,0,0.05)", border: `1px solid ${col}44` }}>{scoreLabel(score)}</span>
                  ...
                  <div style={{ borderRadius: 12, border: "1px solid rgba(0,0,0,0.12)", overflow: "hidden" }}>
                    <div style={{ padding: "10px 16px", background: "#f0f0f0", borderBottom: "1px solid rgba(0,0,0,0.12)", display: "flex", alignItems: "center", gap: 8 }}>
                      <div style={{ width: 3, height: 14, borderRadius: 2, background: COLORS.purple }} />
                      <span style={{ fontSize: 12, fontWeight: 600, color: "#555", letterSpacing: "0.05em" }}>🧬 Brand DNA</span>
                    </div>
                    <div style={{ padding: 16, background: "#fff", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                      {(["positioning", "tone", "uniqueValue", "targetAudience"] as const).map((key) => {
                        ...
                        return (
                          <div key={key} style={{ padding: "10px 14px", borderRadius: 8, background: "#f8f8f8", border: "1px solid rgba(0,0,0,0.1)", borderLeft: `4px solid ${dnaColors[key]}` }}>
                            <div style={{ fontSize: 10, color: "#555", letterSpacing: "0.08em", marginBottom: 4 }}>{labels[key]}</div>
                            <div style={{ fontSize: 13, color: "#333", lineHeight: 1.4 }}>{typeof val === "string" ? val : "—"}</div>
                          </div>
                        );
                      })}
                    </div>
              ...
              {activeTab === "diagnostika" && (
                <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                  <div style={{ borderRadius: 12, border: "1px solid rgba(0,0,0,0.12)", overflow: "hidden" }}>
                    <div style={{ padding: "10px 16px", background: "#f0f0f0", borderBottom: "1px solid rgba(0,0,0,0.12)" }}><div style={{ width: 3, height: 14, borderRadius: 2, background: COLORS.purple, display: "inline-block", marginRight: 8 }} /><span style={{ fontSize: 12, fontWeight: 600, color: "#555" }}>📊 Skóre pilířů</span></div>
                    <div style={{ padding: 16, background: "#fff", display: "flex", flexDirection: "column", gap: 8 }}>
                      {PILLARS.map((p) => {
                        const val = (selected.scan_result as ScanResult)?.pillarAnalysis?.[p.id];
                        const score = typeof val?.score === "number" ? val.score : 5;
                        const col = scoreColor(score);
                        return (
                          <div key={p.id} style={{ display: "flex", alignItems: "center", gap: 12 }}>
                            <div style={{ width: 100, fontSize: 12, color: "#555" }}>{p.icon} {p.label}</div>
                            <div style={{ flex: 1, height: 8, borderRadius: 4, background: "rgba(0,0,0,0.1)", overflow: "hidden" }}><div style={{ height: "100%", width: `${score * 10}%`, background: col, borderRadius: 4 }} /></div>
                            <div style={{ width: 24, fontSize: 13, fontWeight: 700, color: col, textAlign: "right" }}>{score}</div>
                            <span style={{ padding: "2px 7px", borderRadius: 5, fontSize: 10, fontWeight: 600, color: col, background: "rgba(0,0,0,0.05)" }}>{scoreLabel(score)}</span>
                  ...
                  {(selected.scan_result as ScanResult)?.summary && (
                    <div style={{ padding: 16, borderRadius: 10, border: "1px solid rgba(0,0,0,0.12)", background: "#f8f8f8" }}>
                      <div style={{ fontSize: 11, color: "#555", marginBottom: 6 }}>Shrnutí</div>
                      <p style={{ margin: 0, fontSize: 13, color: "#333", lineHeight: 1.6 }}>{(selected.scan_result as ScanResult).summary}</p>
                    </div>
                  )}
                </div>
              )}
              ...
              {activeTab === "strategie" && (
                <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
                  {(selected.scan_result as ScanResult)?.suggested_strategists?.length ? (
                    <div style={{ marginBottom: 12, padding: 12, background: "rgba(168,216,0,0.08)", border: "1px solid rgba(168,216,0,0.3)", borderRadius: 8 }}>
                      <div style={{ fontSize: 11, color: "#555", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 8 }}>AI doporučuje</div>
                      <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>{(selected.scan_result as ScanResult).suggested_strategists!.map((s) => (
                        <button key={s.id} type="button" onClick={() => setStrategistId(s.id)} style={{ padding: "8px 12px", background: "#fff", border: "1px solid rgba(0,0,0,0.12)", borderRadius: 8, color: "#111", fontSize: 12, cursor: "pointer" }}><span style={{ fontWeight: 600 }}>{s.label}</span> <span style={{ marginLeft: 6, color: "#555" }}>→ Vybrat</span></button>
                      ))}</div>
                    </div>
                  ) : null}
                  <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
                    <select value={strategistId} onChange={(e) => setStrategistId(e.target.value)} style={{ padding: "8px 12px", background: "#fff", border: "1px solid rgba(0,0,0,0.12)", borderRadius: 8, color: "#111", fontSize: 13, minWidth: 200 }}>
                  ...
                  {(selected.scan_result as ScanResult)?.strategic_plan != null && (
                    <>
                      ...
                      <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap", marginTop: 12 }}>
                        <input type="text" value={strategyName} onChange={(e) => setStrategyName(e.target.value)} placeholder="Název strategie" style={{ padding: "8px 12px", background: "#fff", border: "1px solid rgba(0,0,0,0.12)", borderRadius: 8, color: "#111", fontSize: 13, minWidth: 180 }} />
                        <button type="button" onClick={saveStrategy} disabled={saveStrategyLoading || !strategyName.trim()} style={{ padding: "8px 16px", background: COLORS.lime, color: "#000", border: "none", borderRadius: 8, cursor: saveStrategyLoading || !strategyName.trim() ? "not-allowed" : "pointer", fontSize: 13, fontWeight: 600 }}>{saveStrategyLoading ? "Ukládám…" : "Uložit jako strategii"}</button>
                      </div>
                    </>
                  )}
                  {((selected.scan_result as ScanResult)?.saved_strategies?.length ?? 0) > 0 && (
                    <div style={{ marginTop: 16 }}>
                      ...
                      <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>{(selected.scan_result as ScanResult).saved_strategies!.map((s) => (
                        <div key={s.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8, padding: "10px 12px", background: "#f8f8f8", border: "1px solid rgba(0,0,0,0.1)", borderRadius: 8, borderLeft: (selected.scan_result as ScanResult)?.active_strategy_id === s.id ? `4px solid ${COLORS.lime}` : "4px solid transparent" }}>
                          <div><span style={{ fontWeight: 600, fontSize: 13, color: "#111" }}>{s.name}</span> <span style={{ marginLeft: 8, fontSize: 12, color: "#666" }}>{new Date(s.created_at).toLocaleDateString("cs-CZ")}</span></div>
                          <button type="button" onClick={() => setActiveStrategy((selected.scan_result as ScanResult)?.active_strategy_id === s.id ? null : s.id)} style={{ padding: "6px 12px", background: (selected.scan_result as ScanResult)?.active_strategy_id === s.id ? "rgba(0,0,0,0.08)" : COLORS.lime, color: (selected.scan_result as ScanResult)?.active_strategy_id === s.id ? "#555" : "#000", border: "none", borderRadius: 6, cursor: "pointer", fontSize: 12, fontWeight: 600 }}>{(selected.scan_result as ScanResult)?.active_strategy_id === s.id ? "Zrušit" : "Použít pro příspěvky"}</button>
                        </div>
                      ))}</div>
                    </div>
                  )}
              ...
              {activeTab === "vystup" && (
                <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                  <div style={{ padding: 16, borderRadius: 10, border: "1px solid rgba(0,0,0,0.12)", background: "#f8f8f8", display: "flex", alignItems: "center", gap: 12 }}>
                    <div style={{ fontSize: 28 }}>🎨</div>
                    <div style={{ flex: 1 }}><div style={{ fontSize: 14, fontWeight: 600, color: "#111", marginBottom: 2 }}>Gamma prezentace</div><div style={{ fontSize: 12, color: "#555" }}>Prezentace strategie — PDF + sdílený odkaz</div></div>
                  ...
                  {gammaStatus === "manual" && gammaResult && (gammaResult.gamma as { gammaInputText?: string })?.gammaInputText && (
                    <div style={{ padding: 12, background: "rgba(0,0,0,0.05)", borderRadius: 8, fontSize: 12, color: "#333" }}><div style={{ fontWeight: 600, color: "#d4a800", marginBottom: 6 }}>✋ Ruční export</div><p style={{ margin: "0 0 8px 0" }}>{(gammaResult.gamma as { instruction?: string }).instruction}</p><button type="button" onClick={() => navigator.clipboard.writeText((gammaResult.gamma as { gammaInputText?: string }).gammaInputText ?? "")} style={{ background: COLORS.lime, color: "#000", border: "none", borderRadius: 6, padding: "6px 12px", fontSize: 12, cursor: "pointer", fontWeight: 600 }}>Kopírovat obsah pro Gamma →</button></div>
                  )}
                  <div style={{ padding: 16, borderRadius: 10, border: "1px solid rgba(0,0,0,0.12)", background: "#f8f8f8", display: "flex", alignItems: "center", gap: 12 }}>
                    <div style={{ fontSize: 28 }}>🎧</div>
                    <div style={{ flex: 1 }}><div style={{ fontSize: 14, fontWeight: 600, color: "#111", marginBottom: 2 }}>NotebookLM průvodce</div><div style={{ fontSize: 12, color: "#555" }}>Audio přehled + AI chat pro klienta</div></div>
                  ...
                  {notebookStatus === "done" && notebookResult && Array.isArray(notebookResult.sources) && (
                    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>{(notebookResult.sources as Array<{ order?: number; title?: string; content?: string }>).map((s) => (
                      <div key={s.order ?? 0} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: 8, background: "rgba(0,0,0,0.05)", borderRadius: 6 }}><span style={{ fontSize: 12, color: "#333" }}>{s.order}. {s.title}</span><button type="button" onClick={() => navigator.clipboard.writeText(s.content ?? "")} style={{ fontSize: 12, color: "#555", background: "none", border: "none", cursor: "pointer" }}>Kopírovat</button></div>
                    ))}</div>
                  )}
                </div>
              )}
              ...
              {activeTab === "poznamky" && (
                <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                  <div style={{ borderRadius: 12, border: "1px solid rgba(0,0,0,0.12)", overflow: "hidden" }}>
                    <div style={{ padding: "10px 16px", background: "#f0f0f0", borderBottom: "1px solid rgba(0,0,0,0.12)" }}><div style={{ width: 3, height: 14, borderRadius: 2, background: COLORS.yellow, display: "inline-block", marginRight: 8 }} /><span style={{ fontSize: 12, fontWeight: 600, color: "#555" }}>📝 Interní poznámky a podklady</span></div>
                    <div style={{ padding: 16, background: "#fff" }}>
                      ...
                      <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap", marginBottom: 16 }}>
                        <input type="text" value={editName} onChange={(e) => setEditName(e.target.value)} placeholder="Jméno nebo název projektu" style={{ flex: "1 1 200px", padding: "10px 14px", background: "#fff", border: "1px solid rgba(0,0,0,0.12)", borderRadius: 8, color: "#111", fontSize: 14, boxSizing: "border-box" }} />
                      ...
                      <textarea value={editInternalNotes} onChange={(e) => setEditInternalNotes(e.target.value)} placeholder="Poznámky kurátora…" rows={3} style={{ width: "100%", padding: "12px 14px", background: "#fff", border: "1px solid rgba(0,0,0,0.12)", borderRadius: 8, color: "#111", fontSize: 14, boxSizing: "border-box", resize: "vertical", marginBottom: 12 }} />
                      <div style={{ fontSize: 11, color: "#555", marginBottom: 6 }}>Podklady (manual_input)</div>
                      <textarea value={editManualInput} onChange={(e) => setEditManualInput(e.target.value)} placeholder="Text zadaný klientem…" rows={4} style={{ width: "100%", padding: "12px 14px", background: "#fff", border: "1px solid rgba(0,0,0,0.12)", borderRadius: 8, color: "#111", fontSize: 14, boxSizing: "border-box", resize: "vertical", marginBottom: 12 }} />
                      <button type="button" onClick={saveDetail} disabled={saving} style={{ padding: "8px 16px", background: COLORS.lime, color: "#000", border: "none", borderRadius: 8, cursor: saving ? "not-allowed" : "pointer", fontSize: 13, fontWeight: 600 }}>{saving ? "Ukládám…" : "Uložit poznámky a podklady"}</button>
                    </div>
                  </div>
                  <div style={{ padding: 16, borderRadius: 10, border: "1px solid rgba(0,0,0,0.12)", background: "#f8f8f8" }}>
