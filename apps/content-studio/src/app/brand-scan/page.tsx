  ...
  return (
    <main className="font-sans" style={{ background: BG, color: TEXT }}>
      <style>{`
        ...
        @media (max-width: 768px) { .bs-entry-grid { grid-template-columns: 1fr !important; } .bs-manual-grid { grid-template-columns: 1fr !important; } }
        ...
        <div style={{ maxWidth: 860, margin: '0 auto' }}>
          ...
          {bsStep === 'entry' ? (
            ...
            <div style={{ animation: 'bsFadeIn 0.3s ease' }}>
              ...
              <div className="bs-entry-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16, marginBottom: 24 }}>
                {[
                  ...
                  >
                    <div style={{ height: 180, overflow: 'hidden', position: 'relative' }}>
                      ...
                      <img src={item.img} alt={item.title} style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block', transition: 'transform 0.4s ease' }} />
                    ...
                    <div style={{ padding: '20px 24px 24px' }}>
                      <div style={{ fontSize: 18, fontWeight: 700, color: TEXT, marginBottom: 8, fontFamily: "'Playfair Display', serif" }}>{item.title}</div>
                      <div style={{ fontSize: 14, color: MUTED, lineHeight: 1.6, marginBottom: 20 }}>{item.desc}</div>
                      <div style={{ background: '#111', color: '#fff', padding: '10px 20px', borderRadius: 10, fontSize: 13, fontWeight: 600, textAlign: 'center' as const }}>Začít →</div>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div style={{ animation: 'bsFadeIn 0.3s ease' }}>
              ...
              <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: '#f0fce0', border: '1px solid rgba(183,233,76,0.4)', borderRadius: 20, padding: '6px 14px', fontSize: 12, color: LIME_DARK, fontWeight: 600, marginBottom: 28 }}>
                {bsEntryType === 'web' && '🌐 Analýza webu'}
                {bsEntryType === 'instagram' && '📱 Analýza Instagramu'}
                {bsEntryType === 'manual' && '✨ Začínám od nuly'}
              </div>
              <div style={{ background: '#fff', border: '1.5px solid #e8e4dc', borderRadius: 20, padding: '36px 40px' }}>
                ...
                {bsEntryType === 'web' && (
                  <div>
                    <h3 style={{ fontFamily: "'Playfair Display', serif", fontSize: 24, fontWeight: 700, color: TEXT, marginBottom: 8, marginTop: 0 }}>Zadej URL svého webu</h3>
                    <p style={{ fontSize: 14, color: MUTED, marginBottom: 24, lineHeight: 1.6 }}>Stačí adresa — AI udělá screenshot, přečte texty a analyzuje značku za ~60 sekund.</p>
                    <input
                      type="url"
                      value={bsInputValue}
                    ...
                    >Analyzovat →</button>
                    <p style={{ textAlign: 'center', fontSize: 11, color: FAINT, marginTop: 12 }}>Analýza trvá ~60 sekund · Zdarma</p>
                  </div>
                )}

                {/* sec-instagram */}
                {bsEntryType === 'instagram' && (
                  <div>
                    <h3 style={{ fontFamily: "'Playfair Display', serif", fontSize: 24, fontWeight: 700, color: TEXT, marginBottom: 8, marginTop: 0 }}>Zadej svůj Instagram</h3>
                    <p style={{ fontSize: 14, color: MUTED, marginBottom: 24, lineHeight: 1.6 }}>Přečteme tvůj profil, styl a vizuální identitu — do pár minut máš přehled.</p>
                    ...
                    <button
                      ...
                      style={{ width: '100%', padding: '13px 0', borderRadius: 12, background: bsInputValue.trim() ? LIME : '#e8e4dc', color: TEXT, border: 'none', fontSize: 15, fontWeight: 700, cursor: bsInputValue.trim() ? 'pointer' : 'not-allowed', transition: 'background .2s' }}
                    >Analyzovat →</button>
                    <p style={{ textAlign: 'center', fontSize: 11, color: FAINT, marginTop: 12 }}>Přečteme tvůj styl a vizuální identitu · Zdarma</p>
                  </div>
                )}
                ...
                {bsEntryType === 'manual' && (
                  <div>
                    <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '.1em', textTransform: 'uppercase' as const, color: LIME_DARK, marginBottom: 8 }}>✨ Začínám od nuly</div>
                    <div style={{ fontFamily: "'Playfair Display', serif", fontSize: 24, fontWeight: 700, color: TEXT, marginBottom: 6 }}>Řekni nám o své značce.</div>
                    <div style={{ fontSize: 14, color: FAINT, marginBottom: 24 }}>Pár kliknutí — žádné dlouhé formuláře.</div>

                    {/* Jméno */}
                    <div style={{ marginBottom: 20 }}>
                      <div style={{ fontSize: 12, fontWeight: 600, color: MUTED, marginBottom: 8, textTransform: 'uppercase' as const, letterSpacing: '.06em' }}>Jak ti říkají?</div>
                      <input
                        type="text"
                        placeholder="Jméno nebo název značky"
                        style={{ width: '100%', border: `1.5px solid ${BORDER}`, borderRadius: 10, padding: '12px 16px', fontSize: 14, fontFamily: 'inherit', outline: 'none', background: '#fff', boxSizing: 'border-box' as const }}
                    ...
                    {/* Co nabízíš */}
                    <div style={{ marginBottom: 20 }}>
                      <div style={{ fontSize: 12, fontWeight: 600, color: MUTED, marginBottom: 10, textTransform: 'uppercase' as const, letterSpacing: '.06em' }}>Co děláš? (vyber 1–2)</div>
                        ...
                        {['Koučink & terapie', 'Kurzy & vzdělávání', 'Kreativní služba', 'Produkty & e-shop', 'Zdraví & péče', 'Konzultace', 'Jiné'].map(t => (
                          <div
                            ...
                            onClick={e => {
                              ...
                              el.style.background = on ? '#fff' : LIME;
                              el.style.color = on ? MUTED : '#1a2a00';
                              el.style.borderColor = on ? BORDER : LIME;
                            }}
                            style={{ padding: '8px 16px', borderRadius: 20, border: `1.5px solid ${BORDER}`, fontSize: 13, cursor: 'pointer', background: '#fff', color: MUTED, transition: 'all .15s', userSelect: 'none' as const }}
                          >{t}</div>
                        ))}
                      </div>
                    </div>
                    ...
                    {/* Pro koho */}
                    <div style={{ marginBottom: 20 }}>
                      <div style={{ fontSize: 12, fontWeight: 600, color: MUTED, marginBottom: 10, textTransform: 'uppercase' as const, letterSpacing: '.06em' }}>Pro koho pracuješ? (vyber 1–2)</div>
                      <div style={{ display: 'flex', flexWrap: 'wrap' as const, gap: 8 }}>
                    ...
                    <div style={{ marginBottom: 24 }}>
                      <div style={{ fontSize: 12, fontWeight: 600, color: MUTED, marginBottom: 10, textTransform: 'uppercase' as const, letterSpacing: '.06em' }}>Cenová hladina?</div>
                          ...
                          <div
                            ...
                            onClick={e => {
                              ...
                              if (p) p.querySelectorAll('div').forEach((d: Element) => {
                                ...
                                (d as HTMLElement).style.color = MUTED;
                                (d as HTMLElement).style.borderColor = BORDER;
                              });
                              const el = e.currentTarget;
                            ...
                            style={{ flex: 1, padding: '10px', borderRadius: 10, border: `1.5px solid ${BORDER}`, fontSize: 13, cursor: 'pointer', background: '#fff', color: MUTED, textAlign: 'center' as const, transition: 'all .15s', userSelect: 'none' as const }}
                          >{t}</div>
                        ))}
                      </div>
                    </div>
                ...
                <StartAnalyzer
                  ...
                  initialUrl={
                    bsEntryType === 'web'
                      ? bsInputValue
                      : bsEntryType === 'instagram'
                      ? `https://www.instagram.com/${bsInputValue.replace(/^@/, '')}`
                      : undefined
                  }
                />
              </div>
